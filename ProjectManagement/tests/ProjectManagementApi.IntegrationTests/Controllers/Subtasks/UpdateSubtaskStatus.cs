...
namespace ProjectManagementApi.IntegrationTests.Controllers.Subtasks
{
    ...
    public class UpdateSubtaskStatus : IntegrationTest
    {
        public UpdateSubtaskStatus(CustomWebApplicationFactory<Startup> factory, ITestOutputHelper output) : base(
            factory, output)
        {
        }

        [Fact]
        public async Task UpdateSubtaskStatusSuccessful()
        {
            var newValue = false;
            var response = await Client.PutAsync(
                $"api/Subtasks/updateStatus/{Utilities.P1S1T1Subtask1.Id}",
            ...
            if (subtask != null)
            {
                subtask.IsDone.Should().Be(newValue);
            }
        }
    }
}